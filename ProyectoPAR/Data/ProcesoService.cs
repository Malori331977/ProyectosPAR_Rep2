...
namespace ProyectoPAR.Data
{
    public class ProcesoService : IProcesoService
    {
        ...
        public async Task<IEnumerable<Reporte>> GetReporte(string estado, string fechaInicio, string fechaFinal, bool facturar, bool facturado)
        {
            IEnumerable<Reporte>? model = null;
            HttpResponseMessage? result = null;
            try
            {
                var method = $"/Reporte/{estado}/{fechaInicio}/{fechaFinal}/{facturar}/{facturado}";
                result = await _genericService.Get(method);
