...
namespace ProyectoPAR.Data
{
    ...
    public partial class ExportData
    {
        ...
        public ExportData(IJSRuntime _JSRuntime)
        {
            JSRuntime= _JSRuntime;
        }
        public IQueryable ApplyQuery<T>(IQueryable<T> items, Radzen.Query? query = null) where T : class
        {
            if (query != null)
            {
                ...
                if (query.Skip is not null && query.Skip>0)
                {
                    items = items.Skip(int.Parse(query.Skip.ToString()!));
                }

                if (query.Top is not null && query.Top > 0)
                {
        ...
        public async Task ToCSV(IQueryable query, string fileName = null)
        {
            ...
            var stream = new MemoryStream(UTF8Encoding.Default.GetBytes($"{string.Join(",", columns.Select(c => c.Key))}{System.Environment.NewLine}{sb.ToString()}"));
        ...
        public async Task ToExcel(IQueryable query, string fileName = null)
        {
            var columns = GetProperties(query.ElementType);
