...
namespace ControlColegiadosBlazorServer.Pages.Certificaciones
{
    ...
    public class ReporteVisita:ReporteBase
    {
        ...
        public static void CreateReport(Document pdf, PdfWriter writer, ReporteExt reporteExt, byte[] logo, Parametro param)
        {
            ...
            p.Add(Text($"Consultor: {reporteExt.NombreConsultor}\n", 15f));
            p.Add(Text($"Hora de Entrada: {reporteExt.HoraEntrada}\nHora de Salida: {reporteExt.HoraSalida}\n", 15f));
            p.Add(Text($"Tiempo Efectivo: {reporteExt.Cantidadhoras} horas\n\n", 15f));


            p.Add(Text($"En la fecha arriba indicada, bajo la presencia del siguiente encargado\n\n", 15f));
            pdf.Add(p);
            ...
            cell = new PdfPCell(Text($"{reporteExt.ResponsableCliente}", 13f));
            ...
            p.IndentationLeft = 58f;
            p.IndentationRight = 58f;
            p.Add(Text($"\n\n\"Doy por aceptadas las labores realizadas y expuestas en este reporte de visita y doy fe de que el total de las horas laboradas son CORRECTAS\".", 15f));
